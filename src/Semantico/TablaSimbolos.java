    ...
    public void imprimirClases() {
        for (Clase c : clases.values()) {
            ...
            for (Metodo m : c.getMetodos().values()) {
                System.out.println(m.getNombre() + ": Tipo: " + m.getRetorno().nombre + ", Forma: " + m.getFormaMetodo());
            }
    ...
    public void imprimirVTs() {
        for (Clase c : clases.values()) {
            ...
            for (Metodo m : c.getMetodos().values()) {
                ...
                if (!m.getNombre().equals(m.getDeclaradoEn().getNombre()) && !m.getFormaMetodo().equals("static")) {
